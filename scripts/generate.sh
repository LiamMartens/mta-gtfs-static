...
for f in $(find ./gtfs_raw -name "*.txt"); do
  ...
  jq --arg PKGJSON_EXPORTS_JSON_TARGET $PKGJSON_EXPORTS_JSON_TARGET --arg PKGJSON_EXPORTS_JSON_TARGET $PKGJSON_EXPORTS_JSON_TARGET \
    '.exports[$PKGJSON_EXPORTS_JSON_TARGET] = $PKGJSON_EXPORTS_JSON_TARGET' \
    ./package.json > ./package.json.tmp && mv ./package.json.tmp ./package.json

  PKGJSON_EXPORTS_JS_TARGET="./gtfs_js/${FILE_DIRNAME}/${FILE_BASENAME%.txt}.js"
  jq --arg PKGJSON_EXPORTS_JS_TARGET $PKGJSON_EXPORTS_JS_TARGET --arg PKGJSON_EXPORTS_JS_TARGET $PKGJSON_EXPORTS_JS_TARGET \
    '.exports[$PKGJSON_EXPORTS_JS_TARGET] = $PKGJSON_EXPORTS_JS_TARGET' \
    ./package.json > ./package.json.tmp && mv ./package.json.tmp ./package.json
